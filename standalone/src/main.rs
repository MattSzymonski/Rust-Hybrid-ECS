//! Standalone host binary — runs the shared [`host`] hot-reload loop either
//! headless or inside a `winit` + `wgpu` window.
//!
//! # Design
//!
//! All DLL build/load/watch/reload logic lives in the [`host`] crate, shared
//! with other host binaries (e.g. `editor`). This binary only adds the
//! windowing/rendering layer: without the `rendering` feature it runs a
//! plain headless loop; with it, it opens a `winit` window and draws every
...
mod windowed {
    ...
    /// GPU resources tied to the window's surface. Boxed and created lazily
    /// in `resumed()`, since `winit` only hands out the window there.
