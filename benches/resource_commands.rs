...
#[derive(Debug)]
struct InputState { mouse_x: f32, mouse_y: f32, buttons: u32 }
impl Resource for InputState {}

#[derive(Debug)]
struct Config { gravity: f32, max_entities: u32 }
...
fn bench_resource_insert(c: &mut Criterion) {
    let mut group = c.benchmark_group("resource_insert");
    for &count in &[100, 1_000, 10_000] {
        group.bench_with_input(BenchmarkId::from_parameter(count), &count, |b, &count| {
            b.iter(|| {
                let mut world = World::new();
                for i in 0..count {
                    world.insert_resource(GameTime { delta: 0.016, elapsed: i as f32 * 0.016 });
                }
...
fn bench_resource_get(c: &mut Criterion) {
    let mut group = c.benchmark_group("resource_get");
    for &count in &[1_000, 10_000, 100_000] {
        group.bench_with_input(BenchmarkId::from_parameter(count), &count, |b, &count| {
            ...
            b.iter(|| {
                for _ in 0..count {
                    black_box(world.get_resource::<GameTime>().map(|resource| resource.elapsed));
                }
            });
        });
    }
...
fn bench_resource_get_mut(c: &mut Criterion) {
    let mut group = c.benchmark_group("resource_get_mut");
    for &count in &[1_000, 10_000, 100_000] {
        group.bench_with_input(BenchmarkId::from_parameter(count), &count, |b, &count| {
            let mut world = World::new();
            world.insert_resource(GameTime { delta: 0.016, elapsed: 0.0 });
...
fn bench_resource_remove(c: &mut Criterion) {
    let mut group = c.benchmark_group("resource_remove");
    for &count in &[100, 1_000, 10_000] {
        group.bench_with_input(BenchmarkId::from_parameter(count), &count, |b, &count| {
            b.iter_batched(
                || {
                    let mut world = World::new();
                    for i in 0..count {
                        world.insert_resource(GameTime { delta: 0.016, elapsed: i as f32 });
                    }
                    world
                },
                |mut world| {
...
fn bench_commands_create_entity(c: &mut Criterion) {
    let mut group = c.benchmark_group("commands_create_entity");
    for &count in &[100, 1_000, 10_000] {
        group.bench_with_input(BenchmarkId::from_parameter(count), &count, |b, &count| {
            b.iter(|| {
...
fn bench_commands_destroy_entity(c: &mut Criterion) {
    let mut group = c.benchmark_group("commands_destroy_entity");
    for &count in &[100, 1_000, 10_000] {
        group.bench_with_input(BenchmarkId::from_parameter(count), &count, |b, &count| {
            b.iter_batched(
                || {
                    ...
                    for i in 0..count {
                        engine.world_mut().create_entity()
                            .with(Position { x: i as f32, y: 0.0 })
                            .build().unwrap();
                    }
                    engine
                ...
                |mut engine| {
                    ...
                    engine.register_system("despawner", move |mut query: Query<Entity>, mut commands: Commands| {
...
fn bench_commands_add_component(c: &mut Criterion) {
    let mut group = c.benchmark_group("commands_add_component");
    for &count in &[100, 1_000, 10_000] {
        group.bench_with_input(BenchmarkId::from_parameter(count), &count, |b, &count| {
            b.iter_batched(
                || {
                ...
                |mut engine| {
                    engine.register_system("adder", move |mut query: Query<(Entity, &Position)>, mut commands: Commands| {
