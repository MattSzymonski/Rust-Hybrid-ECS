...
pub fn generate_function_inventory() {
    ...
    if optimization_level != "0" {
        // The notice is only meaningful when this crate is being built as a
        // hot-loadable module: `module-abi` is the marker the host uses when it
        // builds a loadable cdylib. A static dependency copy in a monolithic
        // release build (the shipping bundle links modules with
        // `default-features = false`) is never patched, so the warning would
        // just be noise on every shipping build.
        if std::env::var_os("CARGO_FEATURE_MODULE_ABI").is_some() {
            println!(
