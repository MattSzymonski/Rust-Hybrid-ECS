//! Offline image-based lighting from an equirectangular panorama.
//!
//! # Responsibilities
//!
//! - Convolves diffuse irradiance and roughness-dependent specular reflections.
//! - Preintegrates a split-sum BRDF lookup texture.
//! - Writes runtime textures sharing the panorama's angular convention.
//!
//! # Design
//!
//! The expensive integration runs in the native cooker. Diffuse output is linear
//! HDR, and the specular mip chain encodes increasing roughness instead of ordinary
//! box-filtered image levels. The BRDF lookup stores linear scale/bias coefficients
//! that the shader combines with material reflectance at runtime.
...
// Current crate
use super::procedural_equirect::{srgb_to_linear, write_rtex, write_rtex_hdr, write_rtex_hdr_mip};
...
/// Environment convolution rule with diffuse output as its freshness anchor.
///
/// Also emits a specular mip chain and a shared `brdf_lut.cooked_tex` beside it.
pub struct EquirectToIBL;

impl Rule for EquirectToIBL {
    fn dependencies(&self) -> &[&str] {
        &["png_to_cooked_tex", "procedural_equirect"]
    }
    ...
    fn build(&self, input: &Path, output: &Path) -> Result<()> {
        ...
        // Step 1: store 32-by-16 diffuse irradiance as linear HDR (the anchor output).
        let irradiance = compute_irradiance(&equirect, 32, 16);
        write_rtex_hdr(output, 32, 16, &irradiance)?;

        // Step 2: store a roughness-prefiltered HDR mip chain (RTEX v4).
        // Five levels span roughness 0.04 through 1; the shader samples roughness * 4.
        ...
        // Step 3: store a 256-by-256 linear BRDF lookup, independent of the panorama.
...
/// CPU panorama sampled in linear light during convolution.
struct Equirect {
    /// Flat linear RGBA channels from the panorama base mip.
    pixels: Vec<f32>,
    /// Number of longitude samples in the base image.
    width: u32,
    /// Number of latitude samples in the base image.
    height: u32,
}

impl Equirect {
    /// Decode the panorama base mip; float values stay linear and bytes are sRGB-decoded.
    fn load(path: &Path) -> Result<Self> {
        let data = std::fs::read(path).with_context(|| format!("read {path:?}"))?;
        let texture = crate::assets::decode_texture(&data).map_err(anyhow::Error::msg)?;
        let width = texture.width;
        let height = texture.height;
        let pixels = if texture.float {
            texture.mips[0]
                .chunks_exact(4)
    ...
    /// Fetch the nearest panorama texel for a world-space direction as linear RGB.
...
/// Map normalized longitude/latitude UVs to a +Y-up unit direction.
...
/// Map a direction to panorama UVs, wrapping longitude and clamping the poles.
...
/// Normalize a vector with a small denominator floor for degenerate input.
...
/// Return the three-dimensional dot product.
...
/// Build orthogonal tangent axes, switching the reference axis near the poles.
...
/// Rotate a local hemisphere direction into a frame centered on the normal.
...
/// Reverse the bits of an index to produce the base-two radical inverse.
...
/// Return a deterministic low-discrepancy two-dimensional integration sample.
...
/// Sample a GGX half-vector around the normal using squared perceptual roughness.
...
/// Integrate the incident hemisphere with cosine and solid-angle weighting.
///
/// The stored result includes the pi factor; the runtime diffuse BRDF divides
/// by pi when applying albedo.
fn compute_irradiance(equirect: &Equirect, output_width: u32, output_height: u32) -> Vec<f32> {
    ...
    for y in 0..output_height {
        for x in 0..output_width {
            ...
            output.push(irradiance[0] * normalization);
            output.push(irradiance[1] * normalization);
            output.push(irradiance[2] * normalization);
            output.push(1.0);
        }
    }
    output
...
/// Convolve the panorama with GGX samples at one roughness level.
///
/// The view direction is taken to equal the normal for the split-sum
/// approximation; positive light contributions are normalized by their weights.
...
/// Approximate one masking term with the roughness remapping used for IBL.
fn geometry_schlick_ggx_ibl(normal_dot_view: f32, roughness: f32) -> f32 {
    let k = roughness * roughness / 2.0;
    normal_dot_view / (normal_dot_view * (1.0 - k) + k)
}

/// Combine independent view and light masking terms for BRDF integration.
...
/// Preintegrate Fresnel scale and bias over view angle and perceptual roughness.
///
/// R stores reflectance scale and G stores bias in linear RGBA8. The horizontal
/// axis is N dot V; the vertical axis increases from smooth to rough.
