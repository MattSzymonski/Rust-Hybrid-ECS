//! Single-mesh OBJ conversion to the RMSH runtime format.
//!
//! # Responsibilities
//!
//! - Triangulates OBJ geometry into a shared vertex/index stream.
//! - Flips texture V and accumulates tangent-space directions from triangle UVs.
//! - Writes the RMSH v1 header and packed vertex/index payloads.
//!
//! # Design
//!
//! The cooked layout stores position, UV, normal, tangent, and bitangent. GPU
//! upload reorders attributes for the shader. Missing OBJ normals remain zero;
//! this converter does not generate normals or import MTL material descriptions.
//! Binary vertex casts assume the native cooker runs on a little-endian host.
...
/// The runtime reader reconstructs the same 14-float sequence from little-endian bytes.
#[repr(C)]
...
impl Rule for ObjToCookedMesh {
    ...
    /// Triangulate one OBJ mesh, average tangent directions, and write RMSH v1.
    ///
    /// Tangents and bitangents are normalized independently; this converter does
    /// not perform Gram-Schmidt orthogonalization against the supplied normals.
    fn build(&self, input: &Path, output: &Path) -> Result<()> {
        // Step 1: request indexed triangles and copy the single mesh into cooked layout.
        ...
        // Step 2: accumulate tangent/bitangent contributions, skipping degenerate UVs.
        ...
        // Step 3: average and independently normalize the accumulated UV directions.
        ...
        // Step 4: write the header followed by packed vertex and index payloads.
