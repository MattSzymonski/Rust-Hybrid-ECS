...
fn degenerate_layouts_are_typed_errors() {
    ...
    assert!(matches!(
        DynamicComponentLayout::new(0, 4, 1, witness),
        Err(WorldError::DynamicSizeZero)
    ));
    assert!(matches!(
        DynamicComponentLayout::new(4, 3, 1, witness),
        Err(WorldError::DynamicAlignmentInvalid)
    ));
    assert!(matches!(
        DynamicComponentLayout::new(usize::MAX, 1, 1, witness),
        Err(WorldError::DynamicLayoutInvalid)
    ));
    ...
    let zero_sized = DynamicComponentLayout {
        size: 0,
        align: 4,
        schema_hash: 1,
    ...
    let misaligned = DynamicComponentLayout {
        size: 4,
        align: 3,
        schema_hash: 1,
        blittability: Blittability::from_manifest_fields(),
...
fn an_unrepresentable_growth_is_a_layout_error() {
    ...
    let unallocatable_growth = DynamicComponentLayout {
        size: usize::MAX / 4 + 8,
        align: 8,
        schema_hash: 1,
        blittability: Blittability::from_manifest_fields(),
