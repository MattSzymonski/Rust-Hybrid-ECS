...
//
// The C# type is AUTO-GENERATED by the host into obj/generated (see
// project_cs.csproj) from the module's real registered layout - nothing is
// hand-written in this project. It is an ABI blob: 16 control points of 3
// floats (12 bytes each) followed by a u32 count, exposed through a safe
// `Raw` span. The probe prints how many splines C# can see - the module seeds
// one, this system creates a second - plus the first control point of the
// first spline, proving the raw bytes are read with the correct layout (the
// module's spline starts at (0,0,0)).
// =============================================================================
...
public static class ModuleSplineBridgeDemo
{
    ...
    public static void Run(Query<Read<global::pill_spline.Spline>> query, Commands commands)
    {
        if (!_seeded)
        {
            ...
            uint count = 4;
            MemoryMarshal.Write(raw.Slice(ControlPointCountOffset), in count);
        ...
        foreach (var row in query)
        {
            Span<byte> bytes = row.Read<global::pill_spline.Spline>().Raw;
            if (visibleSplines == 0)
            {
                firstPointX = MemoryMarshal.Read<float>(bytes.Slice(0));
                firstPointCount = MemoryMarshal.Read<uint>(bytes.Slice(ControlPointCountOffset));
            }
            visibleSplines++;
        }
        Console.WriteLine(
