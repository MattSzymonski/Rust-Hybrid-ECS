...
- `$env:ECS_HOT_RELOAD_MODULE="cs"; cargo run --package standalone` — Build and run the C# game through `cs_runtime` in the standalone host.

- `dotnet build game_cs/game_cs.csproj -c Release --nologo` — Build the C# game and its `cs_runtime` dependency.

- `dotnet run --project cs_runtime/tests/cs_runtime_tests.csproj -c Release` — Run the C# system discovery and scheduler-access tests.
