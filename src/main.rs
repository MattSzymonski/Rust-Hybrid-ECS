...
fn performance_test() {
    ...
    // Create a scene with more entities for testing
    let scene = Scene::new();

    println!("Setting up test world...");

    // Create 100 entities with Transform and Velocity
    for i in 0..10000 {
    ...
    // Create 50 entities with just Transform (no velocity)
    for i in 10000..15000 {
        let entity = scene.instantiate();
        entity
            .add_component(Name::new(format!("Static_{}", i)))
            .add_component(Transform::new(i as f32, 0.0, 0.0));
    }

    scene.apply_commands();

    println!("✓ Created 10000 moving entities + 5000 static entities");
    ...
    for _frame in 0..10_000 {
        let world_lock = scene.world();
        let mut world = world_lock.write();
        executor.execute(&mut world, 0.016); // ~60 FPS delta time
    }

    let duration = start.elapsed();

    println!("\n=== Performance Results ===");
    println!("Total frames:      10,000");
    println!("Total entities:    15000 (10000 moving + 5000 static)");
    println!("Systems per frame: 1 (MovementSystem)");
    println!("\nTime taken:        {:.3} seconds", duration.as_secs_f64());
    println!(
        "Frames per second: {:.0} FPS",
        10_000.0 / duration.as_secs_f64()
    );
    println!(
        "Average frame time: {:.3} ms",
        duration.as_secs_f64() * 1000.0 / 10_000.0
    );
    println!("Entity updates:    {} total", 100 * 10_000);
    println!(
        "Updates per second: {:.0}",
        (100.0 * 10_000.0) / duration.as_secs_f64()
    );
    ...
    // Get first entity position to verify simulation ran
    if let Some(mut query) = world.query::<Transform>() {
        if let Some((entity, transform)) = query.next() {
            if let Some(name) = world.get_component::<Name>(entity) {
                println!("\nSample entity after simulation:");
                println!(
                    "  {} - Position: ({:.2}, {:.2}, {:.2})",
                    name.value, transform.x, transform.y, transform.z
                );
            }
        }
    }
